...
int main() {
  ...
  case 1:
    filename = "input/greeneggsandham.txt";
    break;
  case 2:
    filename = "input/aliceChapter1.txt";
    break;
  case 3:
    filename = "input/romeoandjuliet.txt";
    break;
  case 4:
  ...
  if (!fileIn.is_open()) {
    cerr << "Could not open file \"" << filename
         << "\" for reading.  Shutting down..." << endl;
    return -1;
  }
  ...
  // user wishes to output to a file
    ...
    if (!letterFileOut.is_open()) {
      cerr << "Could not open file \"" << outputFilename
           << "\" for writing.  Shutting down..." << endl;
      return -2;
    }
