...
public:
  ...
  /**
   * @brief inserts an element with provided value before the current element at
   * the provided position.  upon completion, new element will exist at the
   * provided position with all elements after the position shifted over one
   * position
   *
   * @param POS current position to insert in front of
   * @param VALUE value to insert
   * @note if POS is out of range, POS will be clamped to head/tail as
   * appropriate
   */
  virtual void insert(const int POS, const T VALUE) = 0;

  /**
   * @brief removes the element at the provided position.  upon completion, all
   * elements after the position will be shifted over one position
   *
