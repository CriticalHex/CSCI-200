...
/**
 * @brief takes in things and puts them into a list.  stores all the items
 * internally
 */
...
template <typename T> Warehouse<T>::Warehouse() {
...
template <typename T> void Warehouse<T>::store(const T ITEM) {
  _pItems->push_back(new T(ITEM));
}

template <typename T> T &Warehouse<T>::retrieve(const size_t ITEM_POS) const {
  return *(_pItems->at(ITEM_POS));
}

template <typename T> size_t Warehouse<T>::getNumberOfItems() const {
  return _pItems->size();
}
